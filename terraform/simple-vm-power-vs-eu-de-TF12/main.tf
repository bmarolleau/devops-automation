...
data "ibm_pi_public_network" "ds_public_network" {
   depends_on           = [ibm_pi_network.power_network_public]
  pi_cloud_instance_id = var.power_instance_id  
}
...
resource "ibm_pi_instance" "pvminstance" {
  ...
  pi_volume_ids         = []
#  pi_network_ids        = concat(data.ibm_pi_network.power_networks.*.id,[ibm_pi_network.power_network_public.networkid])
 # pi_network_ids        = data.ibm_pi_network.power_networks.*.id 
  pi_network_ids        = concat(data.ibm_pi_network.power_networks.*.id,[data.ibm_pi_public_network.ds_public_network.id]) 
